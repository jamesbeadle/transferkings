...
<div class="relative min-h-[420px] w-full aspect-[1/1.4] group perspective hover:scale-[1.02] transition-transform duration-300">
    <div class={`relative w-full h-full transition-all duration-500 transform-style-3d ${isFlipped ? 'rotate-y-180' : ''}`}>
        <div class="absolute w-full h-full p-8 rounded-2xl backface-hidden bg-{card.color} {card.textColor} transform transition-all duration-200 hover:-translate-y-1 hover:shadow-xl overflow-hidden flex flex-col">
            ...
            <div class="relative z-10 mt-auto">
                ...
                <h3 class="text-3xl font-[1000]">{card.name}</h3>
            </div>
        </div>

        <div class="absolute w-full h-full p-8 rounded-2xl backface-hidden rotate-y-180 bg-{card.color} {card.textColor} flex flex-col">
            <div class="flex items-center justify-between">
                <div class="relative z-20 w-12 h-12">
                    <svelte:component this={card.logo} className="w-full h-full" />
                </div>
            ...
            <div class="flex flex-col h-full pt-4">
                ...
                <h3 class="mb-3 text-2xl font-[1000]">{card.name}</h3>
